...
license: CC-BY-4.0
compatibility: Needs Python 3.10+ with openpyxl and PyYAML for scripts/ (preflight.py,
  k_checker.py, audit.py), all read-only against the workbook. The rates-verification
  date and the install-time question are in metadata and in "Настройка при
  инсталиране" below, not duplicated here.
metadata:
  jurisdiction: BG
